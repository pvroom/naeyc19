			...
			this.databaseprovider.getSearchData(TopicSearchOption, AttendeeID).then(data => {
				...
				if (data['length']>0) {
					...
					for (var i = 0; i < data['length']; i++) {
						...
						var DisplayTitle = data[i].session_title.replace(/\r\\\n/g, '');
						
						var TopicResultData = "<h5>" + DisplayTitle + "</h5><ion-label>" + DisplayDateTime + " in " + data[i].RoomName + "</ion-label>";
						this.sessions.push({
							DisplayEventName: TopicResultData,
