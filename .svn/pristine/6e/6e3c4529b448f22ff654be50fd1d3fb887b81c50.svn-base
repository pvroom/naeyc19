...
export class AppComponent {
	public appMenu = [
		...
		{title: 'Home (Standard)',url: '/home',icon: 'home'},
	...
	async StyleUpdated() {
		const alert = await this.alertCtrl.create({
			header: 'App Styling',
			message: 'The app\'s home screen has been changed.',
			buttons: ['OK']
		});
  	...
  	async SetStyle(StyleType) {
		...
		this.localstorage.setLocalValue('HomeScreenStyle', StyleType);
		this.LoadSideMenuDashboard();
		this.events.publish('user:Status', 'Home screen refresh');
		//await this.StyleUpdated();
		...
		if (AttendeeID !='' && AttendeeID != null) {
			...
			switch(page.title) {
				...
				case "Notes":
					...
					break;
				case "Home (Standard)":
					this.SetStyle('DisplayMenuVertical');
					break;
				case "Home (Dashboard)":
					this.SetStyle('DisplayMenuDashboard');
					break;
				default:
