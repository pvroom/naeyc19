...
@Component({
  selector: 'app-myagenda',
  templateUrl: './myagenda.page.html',
  styleUrls: ['./myagenda.page.scss'],
...
export class MyAgendaPage {
    ...
    LoadData() {
		...
		switch (AgendaDisplayDate) {
			case "11/19/2019":
				...
				AgendaQueryDate = "2019-11-19";
				break;
			case "11/20/2019":
				this.btnWed = "myButtonActive";
				AgendaQueryDate = "2019-11-20";
				break;
			case "11/21/2019":
				this.btnThu = "myButtonActive";
				AgendaQueryDate = "2019-11-21";
				break;
			case "11/22/2019":
				this.btnFri = "myButtonActive";
				AgendaQueryDate = "2019-11-22";
				break;
			case "11/23/2019":
				this.btnSat = "myButtonActive";
				AgendaQueryDate = "2019-11-23";
				break;
			default:
				this.btnTue = "myButtonActive";
				this.storage.set('AgendaDisplayDate', '11/19/2019');
				AgendaQueryDate = "2019-11-19";
				break;
		}
		...
		this.databaseprovider.getAgendaData(flags, AttendeeID).then(data => {
			...
			if (data['length']>0) {
				...
				for (var i1 = 0; i1 < data['length']; i1++) {
					...
					if (data[i1].EventID != 'S-57984' && data[i1].EventID != 'S-57987') {
						...
						idEventShow2 = "agendaEventShow" + agendaStartTime;
											
						if (!this[idEventShow] && !this[idEventShow2]) {
							...
							console.log(idEventShow + ' set to visible');
							...
							if (data[i1].EventID != "0") {
								...
								if ((minutes >= 0) && (minutes <= 30)) {
									...
									this[idEventClass] = "myCard420" + ColumnSelection;
								} else if ((minutes > 420) && (minutes <= 450)) {
									this[idEventClass] = "myCard450" + ColumnSelection;
								} else {
									this[idEventClass] = "myCard120" + ColumnSelection;
								}
								...
								if ((minutes >= 0) && (minutes <= 30)) {
									...
									this[idEventClass] = "myCard420P" + ColumnSelection;
								} else if ((minutes > 420) && (minutes <= 450)) {
									this[idEventClass] = "myCard450P" + ColumnSelection;
								} else {
									this[idEventClass] = "myCard120P" + ColumnSelection;
								}
								console.log(this[idEventClass] + ": " + data[i1].EventName);
							...
							if (!this[idEventShow]) {
								...
								idEventShow = "agendaEventShowW" + agendaStartTime;
								console.log(idEventShow + ' set to hidden');
			...
			console.log("LoadData Promise Rejected");
		});
    ...
    DayUpdate(DayName) {
        ...
        switch (DayName) {
            case "Sat":
				this.localstorage.setLocalValue('AgendaDisplayDate', '11/19/2019');
                break;
            case "Sun":
				this.localstorage.setLocalValue('AgendaDisplayDate', '11/20/2019');
                break;
            case "Mon":
				this.localstorage.setLocalValue('AgendaDisplayDate', '11/21/2019');
                break;
            case "Tue":
				this.localstorage.setLocalValue('AgendaDisplayDate', '11/22/2019');
                break;
            case "Wed":
				this.localstorage.setLocalValue('AgendaDisplayDate', '11/23/2019');
                break;
            default:
				this.localstorage.setLocalValue('AgendaDisplayDate', '11/19/2019');
                break
